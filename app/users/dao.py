...
class UsersDAO(BaseDAO):
    ...
    async def find_all(cls):
        async with async_session_maker() as session:
            query = select(cls.models.__table__.columns).options(
                load_only(
                    Users.id,
                    Users.email,
                    Users.fio,
    ...
    async def find_one_or_none(cls, **filter_by):
        async with async_session_maker() as session:
            query = (
                select(cls.models.__table__.columns)
                .filter_by(**filter_by)
                .options(
                    load_only(
                        Users.id,
    ...
    async def delete_old(cls):
        try:
            async with async_session_taskmaker() as session:
                stmt = delete(cls.models).where(
                    cls.models.created_at
                    + func.make_interval(0, 0, 0, 0, 0, 0, cls.models.expires_in)
                    <= func.current_timestamp(timezone="utc")
                )
